...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    configService = {
      get: jest.fn((key: string) => {
        if (key in configMock) return configMock[key as keyof typeof configMock];
        return undefined;
      }),
    } as unknown as ConfigService;
    service = new AuthService(jwtService, userService, prisma, configService);
  });
  ...
  it('signs in and returns tokens', async () => {
    ...
    jest.spyOn(service as any, 'revokeAllRefreshTokens').mockResolvedValue(undefined);
    jest.spyOn(service, 'createRefreshToken').mockResolvedValue('refresh-token');
    (jwtService.sign as jest.Mock).mockReturnValue('access-token');

    const result = await service.signin(user.email, 'password');

    expect(userService.validateUser).toHaveBeenCalledWith(user.email, 'password');
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        user: expect.objectContaining({ email: user.email, isAuthenticated: true }),
      }),
    );
  });
  ...
  it('creates a refresh token and stores it', async () => {
    const randomSpy = jest
      ...
      .mockImplementation(() => Buffer.from('a'.repeat(32)) as unknown as Buffer);
    ...
    const result = await service.createRefreshToken({ email: user.email, sub: user.id });

    expect(jwtService.sign).toHaveBeenCalledWith(
      { token: '6161616161616161616161616161616161616161616161616161616161616161' },
  ...
  it('verifies refresh token and returns user payload', async () => {
    (jwtService.verify as jest.Mock).mockReturnValue({ token: 'db-refresh-token' });
  ...
  it('refreshes tokens when the access token is invalid and refresh token is valid', async () => {
    ...
    jest.spyOn(service, 'verifyRefreshToken').mockResolvedValue({ user, token: 'refresh-db-token' } as any);
    jest.spyOn(service, 'createRefreshToken').mockResolvedValue('new-refresh-token');
    ...
    const result = await service.verify('expired-access-token', 'valid-refresh-token');
